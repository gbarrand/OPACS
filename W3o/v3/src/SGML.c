...
**	-------------------------------
*/
typedef enum _sgml_state { S_text, S_litteral, S_tag, S_tag_gap, 
		S_attr, S_attr_gap, S_equals, S_value,
		S_ero, S_cro,
		  S_squoted, S_dquoted, S_end, S_entity, S_junk_tag} sgml_state;
...
struct _HTStream {
    ...
    void *		callerData;
#endif
    HTBoolean present[MAX_ATTRIBUTES];	/* Flags: attribute is present? */
