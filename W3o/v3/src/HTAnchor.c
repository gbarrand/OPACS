...
struct _HyperDoc {int dummy;}; /*Barrand:  for VAXC.*/
  ...
  ARGS2 (HTParentAnchor *,parent, CONST char *,tag)
{
  ...
  if (TRACE) fprintf(stderr, "new Anchor %p named `%s' is child of %p\n",
       (void*)child, tag!=NULL ? tag : (CONST char *)"" , (void*)parent); /* int for apollo */
...
PUBLIC HTBoolean HTAnchor_delete
...
PUBLIC HTParentAnchor * HTAnchor_parent
  ARGS1 (HTAnchor *,me)
{
  return me ? me->parent : (HTParentAnchor*)NULL;
}
...
HyperDoc * HTAnchor_document
  ARGS1 (HTParentAnchor *,me)
{
  return me ? me->document : (HyperDoc*)NULL;
}
...
HTFormat HTAnchor_format
  ARGS1 (HTParentAnchor *,me)
{
  return me ? me->format : (HTFormat)NULL;
}
...
HTBoolean HTAnchor_hasChildren
  ARGS1 (HTParentAnchor *,me)
{
...
/*	Make main link
*/
HTBoolean HTAnchor_makeMainLink
