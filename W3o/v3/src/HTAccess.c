...
PUBLIC HTBoolean using_gateway = NO; /* are we using a gateway? */
PUBLIC HTBoolean using_proxy = NO; /* are we using a proxy gateway? */
...
PUBLIC HTBoolean HTRegisterProtocol(HTProtocol* protocol)
{
...
extern HTProtocol HTFTP, HTNews, HTGopher, HTMailto;
#ifdef DIRECT_WAIS
...
char *use_this_url_instead = NULL; /*Barrand*/
...
PUBLIC HTBoolean HTLoadToStream ARGS3(
		CONST char *,	addr,
		HTBoolean, 		filter,
		HTStream *, 	sink)
{
...
PUBLIC HTBoolean HTLoadRelative ARGS2(
