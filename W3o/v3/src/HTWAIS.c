...
#include "HTMLNew.h"		/* The object we will generate */
#include "HTFile.h"
#include "HTAlert.h"
...
PRIVATE HTBoolean acceptable[256];
PRIVATE HTBoolean acceptable_inited = NO;
...
extern "C"{
#endif
char from_hex PARAMS((char a));			/* In HTWSRC @@ */
#ifdef __cplusplus
}
#endif
  ...
  for (p = docid->bytes; 
       ...
       (q < &buf[BIG]);) 
    {
      if (TRACE) fprintf(stderr, "    Record type %d, length %d\n",
                         p[0], p[1]);
      sprintf(num, "%d", (int)*p);
      bcopy_(num, q, strlen(num)); /*Barrand*/
      q += strlen(num);
      p++;
      *q++ = '=';		/* Separate */
	...
	HTStream*,		sink)
{
  ...
  if (key && !*key) 
    ...
    {			/* D O C U M E N T    F E T C H */
      ...
      {
        char *tmp_doctype = strdup_ (doctype);
        char *tmp;
