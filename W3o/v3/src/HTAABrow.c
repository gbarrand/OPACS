					  ...
					  CONST char *, docname)
{
	...
	while (NULL != (setup = (HTAASetup*)HTList_nextObject(cur))) {
	    if (HTAA_templateMatch(setup->template_, docname)) {
		if (TRACE) fprintf(stderr, "%s `%s' %s `%s'\n",
				   "HTAASetup_lookup:", docname,
				   "matched template", setup->template_);
		return setup;
	    }
	    else if (TRACE) fprintf(stderr, "%s `%s' %s `%s'\n",
				    "HTAASetup_lookup:", docname,
				    "did NOT match template", setup->template_);
	} /* while setups remain */
				       ...
				       HTAssocList **,	scheme_specifics)
{
    ...
    if (!server || !template_ || !*template_) return NULL;
    ...
    setup->retry = NO;
    setup->server = server;
    setup->template_ = NULL;
    if (template_) StrAllocCopy(setup->template_, template_);
...
**			NO, otherwise.
*/
PUBLIC HTBoolean HTAA_shouldRetryWithAuth ARGS3(char *, start_of_headers,
					   int,	   length,
					   int,	   soc)
{
    HTAAScheme scheme;
	...
	if (!template_)
	    template_ = HTAA_makeProtectionTemplate(current_docname);
