...
**			NO, if not found.
*/
PUBLIC HTBoolean HTAAMethod_inList ARGS2(HTAAMethod,	method,
				    HTList *,	list)
{
    HTList *cur = list;
    char *item;
...
**			NO, otherwise.
*/
PUBLIC HTBoolean HTAA_templateMatch ARGS2(CONST char *, template_, 
				     CONST char *, filename)
{
...
PUBLIC char *HTAA_makeProtectionTemplate ARGS1(CONST char *, docname)
{
    ...
    if (docname) {
	StrAllocCopy(template_, docname);
	slash = strrchr(template_, '/');
	if (slash) slash++;
	else slash = template_;
	*slash = (char)0;
	StrAllocCat(template_, "*");
    }
    else StrAllocCopy(template_, "*");
