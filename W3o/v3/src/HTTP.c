...
char *post_data = NULL;
extern HTBoolean using_gateway;    /* are we using an HTTP gateway? */
extern HTBoolean using_proxy;      /* are we using an HTTP proxy gateway? */
PUBLIC HTBoolean reloading = NO;   /* did someone say, "RELOAD!?!?!" swp */
	...
	HTStream*,		sink)
{
  ...
  HTBoolean had_header;		/* Have we had at least one header? */
  char *line_buffer;
  char *line_kept_clean;
  HTBoolean extensions;		/* Assume good HTTP server */
  int compressed;
  char line[256];
        ...
        if (line_buffer)
          {
            ...
            bcopy_ (line_buffer, line_kept_clean, buffer_length);
          }
