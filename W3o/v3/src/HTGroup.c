...
*/
PRIVATE HTBoolean part_match ARGS2(CONST char *, tcur,
			      CONST char *, icur)
{
    char required[4];
    char actual[4];
    ...
    if (TRACE) {
	HTBoolean status = HTAA_templateMatch(required, actual);
...
*/
PRIVATE HTBoolean ip_number_match ARGS2(CONST char *,	template,
...
*/
PRIVATE HTBoolean is_domain_mask ARGS1(CONST char *,	mask)
{
    CONST char *cur = mask;
...
*/
PRIVATE HTBoolean ip_mask_match ARGS3(CONST char *,	mask,
...
PRIVATE HTBoolean ip_in_def_list ARGS3(AddressDefList *,	address_def_list,
				  char *,		ip_number,
				  char *,		ip_name)
{
