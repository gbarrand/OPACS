          ...
          *me->value_pointer = 0;
          switch (me->field) 
            ...
            case CONTENT_TYPE:
              ...
              if (TRACE)
                fprintf (stderr, "[MIME_put_char] Got content-type value atom 0x%08lx\n",
                         (unsigned long)me->format);
              break;
	    case CONTENT_TRANSFER_ENCODING:
              me->encoding = HTAtom_for(me->value);
              if (TRACE)
                fprintf (stderr, 
                         "[MIME_put_char] Picked up transfer_encoding 0x%08lx\n",
                         (unsigned long)me->encoding);
              break;
            case CONTENT_ENCODING:
              me->compression_encoding = strdup_ (me->value);
              if (TRACE)
                fprintf (stderr, 
            ...
            case LOCATION:
              ...
              redirecting_url = strdup_ (me->location);
              if (TRACE)
                fprintf
                  (stderr,
  ...
  if (!me->target)
    {
      ...
      if (TRACE)
        fprintf (stderr, "  me 0x%08lx, me->target 0x%08lx\n",(unsigned long)me, (unsigned long)me->target);
      me->format = HTAtom_for ("text/html");
      me->target = HTStreamStack(me->format, me->targetRep, 0,
                                 me->sink, me->anchor);
      if (TRACE)
        fprintf (stderr, "  me->target->isa 0x%08lx\n", (unsigned long)(me->target->isa));
    /*if (TRACE)
        fprintf (stderr, "  *me->target->isa 0x%08lx\n", (unsigned long)(*(me->target->isa)));*/
      me->targetClass = *me->target->isa;
