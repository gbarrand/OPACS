...
/*.........................................................................*/
  ...
  if(a_red!=NULL)    *a_red       = NULL;
  if(a_green!=NULL)  *a_green     = NULL;
  if(a_blue!=NULL)   *a_blue      = NULL;
		...
		int interlace, int ignore)
{
	...
	*/
	...
	image = (unsigned char *)CMemoryAllocateBlock(len * height * sizeof(char));
	if (image == NULL)
	{
#ifdef DEBUG
...
/*.........................................................................*/
  ...
  reds   = (double*)CMemoryAllocateBlock( MAXCOLORMAPSIZE * sizeof(double));
  greens = (double*)CMemoryAllocateBlock( MAXCOLORMAPSIZE * sizeof(double));
  blues  = (double*)CMemoryAllocateBlock( MAXCOLORMAPSIZE * sizeof(double));
  ...
  for(count=0;count<MAXCOLORMAPSIZE;count++)   
    {
      reds   [count] = ((double)a_colors[count].red)/65535.;
      greens [count] = ((double)a_colors[count].green)/65535.;
      blues  [count] = ((double)a_colors[count].blue)/65535.;
    }
  if(a_reds!=NULL)   *a_reds   = reds;   else CMemoryFreeBlock(reds);
  if(a_greens!=NULL) *a_greens = greens; else CMemoryFreeBlock(greens);
  if(a_blues!=NULL)  *a_blues  = blues;  else CMemoryFreeBlock(blues);
...
/*.........................................................................*/
  ...
  CJPEG_BeginGIF_Stream                        (a_file,(long)a_width,(long)a_height);

  colormap           = (JSAMPARRAY)CMemoryAllocateBlock (3 * sizeof(JSAMPROW));
  colormap[0]        = (JSAMPROW) CMemoryAllocateBlock (a_colorn * sizeof(JSAMPLE));
  colormap[1]        = (JSAMPROW) CMemoryAllocateBlock (a_colorn * sizeof(JSAMPLE));
  colormap[2]        = (JSAMPROW) CMemoryAllocateBlock (a_colorn * sizeof(JSAMPLE));
  ...
  CJPEG_PutColormapInGIF_Stream        (a_colorn,colormap);
  CMemoryFreeBlock               (colormap[0]);
  CMemoryFreeBlock               (colormap[1]);
  CMemoryFreeBlock               (colormap[2]);
  CMemoryFreeBlock               (colormap);

  pixel_data[0] = (JSAMPARRAY)CMemoryAllocateBlock (a_height * sizeof(JSAMPROW));

  data = a_data;
  for(row=0;row<a_height;row++)
    {
      pixel_data[0][row] = (JSAMPROW)CMemoryAllocateBlock (a_width * sizeof(JSAMPLE));
  ...
  CJPEG_PutImageInGIF_Stream       ((long)a_width,(int)a_height,pixel_data);

  for(row=0;row<a_height;row++) CMemoryFreeBlock (pixel_data[0][row]);
  CMemoryFreeBlock               (pixel_data[0]);

  CJPEG_EndGIF_Stream                ();
}
/***************************************************************************/
void CGIF_PutImageInStream (
 FILE* a_file
,CGIF_GetPixelFunction a_proc
,unsigned int a_width
,unsigned int a_height
,int a_colorn
,double* a_red
