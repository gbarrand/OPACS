 ...
 * white space.
 */
struct mark_up * AddObj(struct mark_up **listp,struct mark_up *current,struct mark_up *mark,int keep_wsp)
{
	if (mark == NULL)
	{
		return(current);
 ...
 * return a pointer to the current (end) position in the list.
 */
struct ele_rec * AddEle(struct ele_rec **elistp,struct ele_rec *current,struct ele_rec *eptr)
{
	if (eptr == NULL)
	{
		return(current);
 ...
 * Otherwise return 0.
 */
int ElementLessThan(struct ele_rec *start,struct ele_rec *end,int start_pos, int end_pos)
{
	struct ele_rec *current;
 ...
 * Otherwise return 0.
 */
int SwapElements(struct ele_rec *start,struct ele_rec *end,int start_pos, int end_pos)
{
	struct ele_rec *current;
