...
static ImageInfo no_image       ;  /*Barrand*/
static ImageInfo delayed_image  ;  /*Barrand*/
static ImageInfo anchored_image ;  /*Barrand*/
 ...
 */
XImage * MakeImage(Display *dsp,unsigned char *data,int width, int height,int depth,ImageInfo *img_info)
{
	...
	switch(depth)
	{
		...
		bcopy_ ((char*)data, (char*)bit_data, (width * height)); /*Barrand*/
		bytesperline = width;
...
ImageInfo * DelayedImageData(HTMLWidget hw,Boolean anchored)
{
...
Pixmap InfoToImage(HTMLWidget hw,ImageInfo *img_info)
{
	int i, size;
	...
	unsigned long black_pixel = 0L;
	unsigned long white_pixel = 0L;
