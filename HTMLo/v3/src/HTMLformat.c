...
extern TableInfo *MakeTable (HTMLWidget hw,struct mark_up **mptr,unsigned int x,unsigned int y);
 ...
 */
struct ele_rec * CreateElement(HTMLWidget hw,	int type,XFontStruct *fp,int x, int y,char *edata)
{
	struct ele_rec *eptr;
	int baseline;
 ...
 */
		...
		case E_TABLE:
			...
			eptr->table_data = MakeTable (hw, (struct mark_up **)edata,
 ...
 */
void SetElement(HTMLWidget hw,int type,XFontStruct *fp,int x, int y,char *edata)
{
	struct ele_rec *eptr;
	int len;
	int baseline;
 ...
 */
void LinefeedPlace(HTMLWidget hw,int *x, int *y)
{
	/*
 ...
 */
void LineFeed(HTMLWidget hw,int *x, int *y)
{
	/*
 ...
 */
void ConditionalLineFeed(HTMLWidget hw,int *x, int *y,int state)
{
 ...
 * looking linefeeds.
 */
void HardLineFeed(HTMLWidget hw,int *x, int *y)
{
	/*
	 * Manipulate linefeed state for special pre-formatted linefeed
	 * hack for broken HTMLs
        ...
        if ((Superscript>0) || (Subscript>0))
          {
            ...
            baseline += (int)((supsubBaseline * .4) * Superscript);
            baseline -= (int)((supsubBaseline * .4) * Subscript);
            baseline += 2;
          }
 ...
 */
void BulletPlace(HTMLWidget hw,int *x, int *y)
{
	int width, l_height;
 ...
 * Create and add the element record for it.
 */
void HRulePlace(HTMLWidget hw,int *x, int *y,unsigned int width)
{
	NeedSpace = 0;
 ...
 * list item. Create and add the element record for it.
 */
void ListNumberPlace(HTMLWidget hw,int *x, int *y, int val)
{
	int width, my_x;
 ...
 */
void PreformatPlace(HTMLWidget hw,struct mark_up *mptr,int *x, int *y,unsigned int width)
{
	char *text;
	char *start;
	char *end;
 ...
 */
void FormatPlace(HTMLWidget hw,struct mark_up *mptr,int *x, int *y,unsigned int width)
{
	char *text;
	char *start;
	char *end;
...
void TablePlace(HTMLWidget hw,struct mark_up **mptr,int *x, int *y,unsigned int width)
{
int extra;

#ifdef DEBUG
	printf("TablePlace(hw,mptr,*x=%d,*y=%d,width = %d)\n", *x,*y,width);
#endif
 ...
 */
void ImagePlace(HTMLWidget hw,struct mark_up *mptr,int *x, int *y,unsigned int width)
{
	char *tptr;
 ...
 */
void WidgetPlace(HTMLWidget hw,struct mark_up *mptr,int *x, int *y,unsigned int width)
{
 ...
 */
static void TriggerMarkChanges(HTMLWidget hw,struct mark_up **mptr,int *x, int *y)
{
	struct mark_up *mark;
	XFontStruct *font;
	int type, width;
 ...
 */
void FormatChunk(HTMLWidget hw,int *x, int *y)
{
	struct mark_up *mptr;
 ...
 */
void PartialRefresh(HTMLWidget hw,struct ele_rec *eptr,int start_pos, int end_pos,unsigned long fg, unsigned long bg)
{
	int ascent;
	char *tdata;
	int tlen;
 ...
 */
void TextRefresh(HTMLWidget hw, struct ele_rec * eptr, int start_pos, int end_pos)
{
...
void RefreshTextRange(HTMLWidget hw, struct ele_rec * start, struct ele_rec * end)
{
	struct ele_rec *eptr;
...
void strcpy_or_grow(char **str,	int *slen,int *blen,char *add)
{
	...
	bcopy_(add, (char *)(buf + *slen), addlen + 1);
 ...
 * to indent lines.
 */
char * ParseTextToString(struct ele_rec *elist,struct ele_rec *startp,struct ele_rec *endp,int start_pos, int end_pos,int space_width,int lmargin)
{
	int newline;
	int epos;
	char *text;
 ...
 * to indent lines.
 */
char * ParseTextToPrettyString (HTMLWidget hw,struct ele_rec *elist,struct ele_rec *startp,struct ele_rec *endp,int start_pos, int end_pos,int space_width,int lmargin)
{
	int line;
	int newline;
	int lead_spaces;
