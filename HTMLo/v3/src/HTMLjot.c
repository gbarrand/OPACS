...
#if defined(VMS) && defined(__alpha)       /*Barrand*/
#pragma __nostandard
#define htonl decc$htonl                   /*Barrand*/
unsigned int decc$htonl(unsigned int);     /*Barrand*/
#pragma __standard
#endif
...
void NewJot(Widget w,int width, int height)
{
	if (JotCurrent == NULL)
	{
...
void ClearJot(HTMLWidget hw,Widget w,int width, int height)
{
	JotInfo *jptr;
...
void AddStroke(JotInfo *jptr,Stroke *sptr,Boolean drawing)
{
...
void EVJotExpose(Widget w,XtPointer data,XEvent *event,Boolean* continue_to_dispatch)
{
	...
	XCopyArea(XtDisplay(w), jptr->pix, XtWindow(w),
                        ...
                        ExEvent->x, ExEvent->y);
	continue_to_dispatch = NULL;
}


void EVJotPress(Widget w,XtPointer data,XEvent *event,Boolean* continue_to_dispatch)
{
...
void EVJotMove(Widget w,XtPointer data,XEvent *event,Boolean* continue_to_dispatch)
{
...
void EVJotRelease(Widget w,XtPointer data,XEvent *event,Boolean* continue_to_dispatch)
{
...
char * EJB_JOTfromJot(Widget w)
{
	...
	while ((sptr != NULL)&&(cnt < jptr->stroke_cnt))
	{
		...
		bcopy_((char *)&val, (char *)dptr, sizeof(u_long));
...
unsigned char * JOTfromJot(Widget w,int *buffer_len)
{
	int i, cnt;
	int dlen, total;
	JotInfo *jptr;
	...
	bcopy_((char *)dataArray, (char *)pptr->inkData, dlen);
	free((char *)dataArray);
/*
...
*/
	...
	total = inkRecordBundleSize + inkRecordPenDataSize(dlen) +
		inkRecordEndSize;
	buffer = (unsigned char *)malloc(total);
	bcopy_((char *)iptr, (char*)buffer, inkRecordBundleSize);
	bcopy_((char *)pptr, (char *)(buffer + inkRecordBundleSize),
		inkRecordPenDataSize(dlen));
	bcopy_((char *)eptr, (char *)(buffer + inkRecordBundleSize +
