...
extern int caseless_equal(char*,char*); /*Barrand*/
extern void clean_text (char*); /*Barrand*/
...
int y;
...
#ifdef DEBUG
	printf("PourText: \"%s\" width=%d,height=%d\n",text,width,height);
#endif
	if (!text) {
		*height = 0;
	...
	if (stringWidth < width) {
		ListAddEntry(textList,strdup_(text));
		}
	else {
...
printf("textPtr = \"%s\"\n",textPtr);
		...
		if ((builtWidth + spaceWidth + wordWidth)  < width) { 
			...
			/* start new line */
			ListAddEntry(textList, strdup_(tmpBuff));
			*tmpBuff = '\0';
			builtWidth = 0;
...
static int TableCleanUp(TableInfo *t,List tableList)
{
int maxNumCols;
int numColsInRow;
List rowList;
...
int x,y;
	...
	while(rowList) {
		...
		while(numColsInRow < t->numColumns) {
			...
			ListAddEntry(rowList,(char*)field);
			numColsInRow++;
			}
		rowList = (List) ListNext(tableList);
		}
...
static int TableHowManyConnectedHorizFields(TableInfo *t,int xpos,int ypos)
{
int count;
register int x;
...
/* return the number of connected fields */
static int TableHowManyConnectedVertFields(TableInfo *t,int xpos,int ypos)
{
int count;
register int y;
...
static int CalculateMaxWidthOfColumn(TableInfo *t,int x)
{
register int y;
...
void TableCalculateDimensions(HTMLWidget hw,TableInfo *t,int pageWidth)		/* width in pixels of output page */
{
TableField *field;
register int xx,x,y;
	...
	if (sumMaxWidth < pageWidth ) {
		...
		for (y=0; y < t->numRows; y++) {
			for (x=0; x < t->numColumns; x++) {
			    ...
			    if (field->text) {
				field->formattedText[0]= strdup_(field->text);
				field->numLines = 1;
				}
			    else {
...
*/
			...
			for (y = 0; y < t->numRows; y++) {
				...
				for (xx = x+1; xx < x+numAdjacent+1; xx++) {
				    accumulateColWidth += (int)(
					(percentToShrink *
                                        ((float) CalculateMaxWidthOfColumn(t,xx))));
				    }
				
#ifdef DEBUG
printf("About to call PourText\n");
#endif
				PourText(field->text,field->font,
					...
					&(field->numLines));
#ifdef DEBUG
printf("poured field %d,%d is dims %d,%d: %%shrink=%f\n",
...
static Boolean TableExpandFields(List tableList,List rowList,int rowCount,int *columnCount)
{
TableField *field;
...
static void TableFieldSetAttributes(HTMLWidget hw,TableField *field,struct mark_up **mptr)
{
struct mark_up *m;
/*
Boolean bold;
...
TableInfo *MakeTable (HTMLWidget hw,struct mark_up **mptr,unsigned int x,unsigned int y)
{
struct mark_up *m;
TableInfo *t;
TableField *field;
...
List tableList;			/* list of Row Lists */
	...
	if (ParseMarkTag(((*mptr)->start),MT_TABLE,"border")) {
		t->borders = 1;
		}
	else {
		t->borders = 0;
	...
	rowList = ListCreate();
	ListAddEntry(tableList, (char*)rowList);
	columnCount = 0;
	rowCount=1;
	m = *mptr;
	field = (TableField *) 0;
	while (m && (!((m->type == M_TABLE) && (m->is_end)))) {
		...
		if (m->type == M_CAPTION) {
			...
			/* if: is this the first container <tr> or the 
			       separator */
			if (ListHead((List)ListHead(tableList))) {
				rowList = ListCreate();
				ListAddEntry(tableList,(char*)rowList);
				rowCount++;
				}
...
*/
	...
	while (rowList) {
		field = (TableField *) ListHead(rowList);
		while(field) {
			ListDeleteEntry(rowList,(char*)field);
			free(field);
			field = (TableField *) ListHead(rowList);
			}
		ListDeleteEntry(tableList,(char*)rowList);
		ListDestroy(rowList);
		rowList = (List) ListHead(tableList);
		}
...
void TableDisplayField(
 HTMLWidget hw
,struct ele_rec *eptr
,TableField *field
,int x,int y /* field origin */
,int width,int height /* space allowed for displaying */
)
{
...
static void TableGetExpandedDimensions(
 TableInfo *t
,int xpos,int ypos /* current field index */
,int *expandWidth,int *expandHeight /* returned */
)
{
int x,y;
	x = xpos;
...
*/
			...
			/* fill in field */
			TableDisplayField(hw,eptr,field,vertMarker,horizMarker,
