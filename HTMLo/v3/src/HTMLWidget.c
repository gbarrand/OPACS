...
extern "C"{
#endif
extern void _XmDrawShadow(Display*,Drawable,GC,GC,Dimension,Position,Position,Dimension,Dimension); /*Barrand*/
#if (XmVERSION >= 2)
...
extern void BulletRefresh(HTMLWidget,struct ele_rec*); /*Barrand*/
extern int FormatAll(HTMLWidget hw, int* Fwidth);
extern int DocumentWidth(HTMLWidget, struct mark_up *);
extern void PlaceLine(HTMLWidget, int);
...
static void		_HTMLpwdInput(Widget w, XEvent *event,
				String *params, Cardinal *num_params);
#endif
static void             Initialize(HTMLWidget request, HTMLWidget new_);
...
static void             DestroyWidget    (HTMLWidget hw);
...
/* amb */
        {       WbNsupSubFont,
                ...
                XtRString, "-adobe-courier-medium-r-normal-*-11-*-*-*-*-*-*-*"  /*Barrand 10->11*/
        },
/* end amb */
...
HTMLClassRec htmlClassRec = {
   {						/* core class fields  */
      ...
      (XtWidgetProc) DestroyWidget,		/* destroy            */
 ...
 */
static void
DrawExpose(Widget w, XtPointer data, XEvent* event)
{
	...
	while (((XCheckWindowEvent(XtDisplay(w), XtWindow(w), ExposureMask, &NewEvent) == True))) {
 ...
 */
static void
setScrollBar(Widget sb, Position topPosition, Dimension totalLength, Dimension currentLength)
{
 ...
 */
void
ScrollMove(Widget w, XtPointer client_data, XtPointer call_data)
{
#ifdef MOTIF
	...
	if (w == hw->html.vbar)
	{
		...
		value = (int)(hw->html.scroll_y + scrollDir * currentLength);
	}
	else 
	{
		totalLength = hw->html.doc_width;
		currentLength = hw->html.view_width;
		value = (int)(hw->html.scroll_x + scrollDir * currentLength);
	}

	if (value > (int)totalLength) value = totalLength;
	if (value < 0) value = 0;
  
	setScrollBar(w, (Position)value, totalLength, currentLength);
...
#ifndef MOTIF
void
JumpMove(Widget w, XtPointer client_data, XtPointer call_data)
{
	HTMLWidget hw = (HTMLWidget)client_data;
...
static void CreateScrollbars(HTMLWidget hw)
{
	...
	XtAddCallback(hw->html.vbar, XtNjumpProc,
		(XtCallbackProc)JumpMove, (XtPointer)hw);
...
fprintf (stderr, "computed ss to be %d\n", ss);
		...
		setScrollBar(hw->html.vbar, 
			     (Position)hw->html.scroll_y,
			     (Dimension)hw->html.doc_height, 
			     hw->html.view_height);
#endif /* MOTIF */
...
fprintf (stderr, "real slider size %d\n", ss);
	...
	if (hw->html.use_hbar == True)
	{
		...
		setScrollBar(hw->html.hbar, 	
			     (Position)hw->html.scroll_x,
			     (Dimension)hw->html.doc_width, 
			     hw->html.view_width);
#endif /* MOTIF */
	}
            ...
            HTMLWidget neww)
{
	...
	{
		neww->core.width = neww->html.margin_width << 1 ;
	} 
	if (neww->core.width == 0)
	{
		neww->core.width = 10 ;
	...
	{
		neww->core.height = neww->html.margin_height << 1 ;
	} 
	if (neww->core.height == 0)
	{
		neww->core.height = 10 ;
	 ...
	 */
	if (neww->html.num_anchor_underlines < 0)
	{
		neww->html.num_anchor_underlines = 0;
	} 
	if (neww->html.num_anchor_underlines > MAX_UNDERLINES)
	{
		neww->html.num_anchor_underlines = MAX_UNDERLINES;
	} 
	if (neww->html.num_visitedAnchor_underlines < 0)
	{
		neww->html.num_visitedAnchor_underlines = 0;
	} 
	if (neww->html.num_visitedAnchor_underlines > MAX_UNDERLINES)
	{
		neww->html.num_visitedAnchor_underlines = MAX_UNDERLINES;
	} 
	 ...
	 */
	neww->html.html_objects = HTMLParse(NULL, request->html.raw_text);
	CallLinkCallbacks(neww);
	neww->html.html_header_objects =
		HTMLParse(NULL, request->html.header_text);
	neww->html.html_footer_objects =
		HTMLParse(NULL, request->html.footer_text);
	neww->html.formatted_elements = NULL;
	neww->html.my_visited_hrefs = NULL;
	neww->html.my_delayed_images = NULL;
	neww->html.widget_list = NULL;
	neww->html.form_list = NULL;

	/*
	 * Blank document
	 */
	neww->html.line_array = NULL;
	neww->html.line_count = 0;

	/*
	 * Find the max width of a preformatted
	 * line in this document.
	 */
	neww->html.max_pre_width = DocumentWidth(neww, neww->html.html_objects);
	 ...
	 * if narrow enough.
	 */
	CreateScrollbars(neww);
	neww->html.scroll_x = 0;
	neww->html.scroll_y = 0;
	ReformatWindow(neww);

	/*
	 * Initialize private widget resources
	 */
	neww->html.drawGC = NULL;
	neww->html.select_start = NULL;
	neww->html.select_end = NULL;
	neww->html.sel_start_pos = 0;
	neww->html.sel_end_pos = 0;
	neww->html.new_start = NULL;
	neww->html.new_end = NULL;
	neww->html.new_start_pos = 0;
	neww->html.new_end_pos = 0;
	neww->html.active_anchor = NULL;
	neww->html.press_x = 0;
	neww->html.press_y = 0;

        neww->html.cached_tracked_ele = NULL;

        /* Initialize cursor used when pointer is inside anchor. */
        if (in_anchor_cursor == (Cursor)NULL)
          in_anchor_cursor = XCreateFontCursor (XtDisplay (neww), XC_hand2);
...
/* Destroy callback to clear things. */
static void DestroyWidget (HTMLWidget This)
{
/*.........................................................................*/
  HTMLParse(This->html.html_objects,NULL);
  This->html.html_objects = NULL;
}
 ...
 * from start to end.
 */
static void DrawSelection(HTMLWidget hw,struct ele_rec *start,struct ele_rec *end,int start_pos, int end_pos)
{
	struct ele_rec *eptr;
	int epos;
 ...
 */
static void EraseSelection(HTMLWidget hw,struct ele_rec *start,struct ele_rec *end,int start_pos, int end_pos)
{
	struct ele_rec *eptr;
	int epos;
 ...
 * flashing.
 */
static void ChangeSelection(HTMLWidget hw,struct ele_rec *start,struct ele_rec *end,int start_pos, int end_pos)
{
		...
		if (eptr != NULL)
		{
			if (eptr->anchorHRef != NULL)
			{
			    ...
			    */
			    ...
			    {
				eptr->pic_data = (*(resolveDelayedImageProc)
...
/*	int reformatted;*/
	    ...
	    (request->html.footer_text != current->html.footer_text))
	{
		 ...
		 */
		neww->html.html_objects = HTMLParse(current->html.html_objects,
		 ...
		 */
		{
			neww->html.scroll_x = 0;
			neww->html.scroll_y = 0;
			neww->html.max_pre_width = DocumentWidth(neww,
				neww->html.html_objects);
			ReformatWindow(neww);
			ViewClearAndRefresh(neww);
			/*reformatted = 1;*/
		}
...
static Boolean ConvertSelection(Widget w,Atom *selection, Atom *target, Atom *type,XtPointer *value,unsigned long *length,int *format)
{
	...
	if (*target == XA_TARGETS(d))
	{
		...
		*targetP++ = XA_LIST_LENGTH(d);
		...
		bcopy_((char*)std_targets, (char*)targetP,
	...
	if (*target == XA_LIST_LENGTH(d))
	{
		*value = XtMalloc(4);
		...
		{
			long temp = 1;
			bcopy_( ((char*)&temp)+sizeof(long)-4, (char*)*value, 4);
		}
		*type = XA_INTEGER;
		*length = 1;
		*format = 32;
 ...
 */
char* HTMLGetText(Widget w, int pretty, char *url)
{
	HTMLWidget hw = (HTMLWidget)w;
	char *text;
	char *tptr, *buf;
 ...
 */
void HTMLGotoId(Widget w, int element_id)
{
	HTMLWidget hw = (HTMLWidget)w;
	struct ele_rec *start;
	struct ele_rec *eptr;
...
void HTMLFreeImageInfo(Widget w)
{
	HTMLWidget hw = (HTMLWidget)w;
 ...
 * to that anchor.
 */
void HTMLSetText(Widget w, char *text, char *header_text, char *footer_text, int element_id, char *target_anchor, void *ptr)
{
	HTMLWidget hw = (HTMLWidget)w;
	WidgetInfo *wptr = (WidgetInfo *)ptr;
	struct ele_rec *start;
 ...
 */
int HTMLSearchText (Widget w, char *pattern, ElementRef *m_start, ElementRef *m_end,
