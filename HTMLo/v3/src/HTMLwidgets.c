...
static int strcasecmp_ (char* a,char* b); /*Barrand unknown in VMS string.h*/

extern void NewJot(Widget w,int width, int height);
extern void ClearJot(HTMLWidget hw,Widget w,int width, int height);
extern void EVJotExpose(Widget w,XtPointer data,XEvent *event,Boolean* continue_to_dispatch);
extern void EVJotPress(Widget w,XtPointer data,XEvent *event,Boolean* continue_to_dispatch);
extern void EVJotMove(Widget w,XtPointer data,XEvent *event,Boolean* continue_to_dispatch);
extern void EVJotRelease(Widget w,XtPointer data,XEvent *event,Boolean* continue_to_dispatch);
...
void setTextSize(Widget w,int columns,int lines)
{
	XFontStruct *font;
...
void CBListDestroy(Widget w,XtPointer client_data,XtPointer call_data)
{
...
void CBTextDestroy(Widget w,XtPointer client_data,	XtPointer call_data)
{
...
void CBoption(Widget w,XtPointer client_data,XtPointer call_data)
{
 ...
 */
XFontStruct * GetWidgetFont(HTMLWidget hw,WidgetInfo *wptr)
{
	Widget child;
	XFontStruct *font;
#ifdef MOTIF
 ...
 */
WidgetInfo * MakeWidget(HTMLWidget hw,char *text,int x, int y,int id,	FormInfo *fptr)
{
	Arg arg[30];
	Cardinal argcnt;
	Widget w;
	...
	if (wlist == NULL)
	{
		...
		if ((type_str != NULL)&&(strcasecmp_(type_str, "checkbox") == 0))
		{
			XmString label;
		...
		else if ((type_str != NULL)&&(strcasecmp_(type_str, "hidden") == 0))
		{
			type = W_HIDDEN;
			value = ParseMarkTag(text, MT_INPUT, "VALUE");
			if (value == NULL)
		...
		else if ((type_str != NULL)&&(strcasecmp_(type_str, "radio") == 0))
		{
			XmString label;
		...
		else if ((type_str != NULL)&&(strcasecmp_(type_str, "submit") == 0))
		{
			XmString label;
		...
		else if ((type_str != NULL)&&(strcasecmp_(type_str, "reset") == 0))
		{
			XmString label;
		...
		else if ((type_str != NULL)&&(strcasecmp_(type_str, "button") == 0))
		{
			XmString label;
		...
		else if ((type_str != NULL)&&(strcasecmp_(type_str, "jot") == 0))
		{
			XmString label;
			Dimension width, height;
			Widget frame;
		...
		else if ((type_str != NULL)&&(strcasecmp_(type_str, "select") == 0))
		{
			XmString label;
			Widget scroll;
			...
			if ((tptr != NULL)&&(strcasecmp_(tptr, "list") == 0))
			{
				type = W_LIST;
			}
			else if ((tptr != NULL)&&(strcasecmp_(tptr, "menu") == 0))
			{
				type = W_OPTIONMENU;
			}
			if (tptr != NULL)
		...
		else if ((type_str != NULL)&&(strcasecmp_(type_str, "password") ==0))
		{
			type = W_PASSWORD;
			value = ParseMarkTag(text, MT_INPUT, "VALUE");
...
#endif /* MOTIF1_2 */
			XtAddCallback(w, XmNactivateCallback,
				(XtCallbackProc)CBActivateField, (XtPointer)fptr);
			XtAddCallback(w, XmNmodifyVerifyCallback,
				(XtCallbackProc)CBPasswordModify, (XtPointer)fptr);
		}
		else if ((type_str != NULL)&&(strcasecmp_(type_str, "textarea") ==0))
		{
			char **list;
			int list_cnt;
			int rows, cols;
...
static int strcasecmp_ (char* a,char* b)
{
	char *p =a;
	char *q =b;
	for(p=a, q=b; *p && *q; p++, q++) {
