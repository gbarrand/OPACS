...
#include <X11/StringDefs.h>/*barrand*/

static XColor fg_color, bg_color; /*barrand*/
...
extern "C"{
#endif
unsigned char *ReadGIF        (FILE*,int*,int*,XColor*,int*);
unsigned char *ReadXpm3Pixmap (FILE*,char*,int*,int*,XColor*,int*);
#ifdef __cplusplus
}
#endif
...
/*barrand: extern XColor fg_color, bg_color;*/
        extern Widget view;
        extern int Vclass;
	int blackbit;
	int whitebit;
        ...
        if (Rdata.reverse_inlined_bitmap_colors)
          {
            blackbit = (int)bg_color.pixel;
            whitebit = (int)fg_color.pixel;
          }
        else
          {
            blackbit = (int)fg_color.pixel;
            whitebit = (int)bg_color.pixel;
          }
  
	/*
	 * ReportError out here on visuals we can't handle so we won't core dump
	 * later.
	 */
	if (((blackbit > 255)||(whitebit > 255))&&(Vclass != TrueColor))
	  {
		fprintf(stderr, "ReportError:  cannot deal with default colormap that is deeper than 8, and not TrueColor\n");
	...
	*h = 0;
	...
	if (version10p)
	{
		...
		for (bytes = 0; bytes < raster_length; bytes += 2)
		{
			if (fscanf(fp, " 0x%x%*[,}]%*[ \r\n]",&value) != 1)
			{
				fprintf(stderr, "ReportError scanning bits item.\n");
				return((unsigned char *)NULL);
			}
			temp = value;
			value = temp & 0xff;
