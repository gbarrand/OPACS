 ...
 */
void
xpmInitInternAttrib (xpmInternAttrib *attrib)
{
    attrib->ncolors = 0;
 ...
 */
void
xpmFreeInternAttrib (xpmInternAttrib *attrib)
{
...
#endif
		    if (!(mdata->stream.file = fopen(filename, "rb"))) {
#ifdef ZPIPE
			XpmFree(compressfile);
#endif
			return (XpmOpenFailed);
...
/*barrand*/ bzero((char*)colidx, 256 * sizeof(short));
	    for (a = 0; a < ncolors; a++)
...
/*barrand*/ bzero((char*)cidx, 256 * 256 * sizeof(short));
	    for (a = 0; a < ncolors; a++)
...
unsigned char *ReadXpm3Pixmap ( /*barrand*/
FILE *fp
,char *datafile
,int *w
,int *h
